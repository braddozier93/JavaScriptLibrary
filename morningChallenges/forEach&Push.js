...
//within it. Using the forEach method, loop over all of the presidents from our presidents array. 
//Then, push the president's last name and the year they were born to a new array as individual objects
// containing key/value pairs of each presidents last name and date of birth. Then, return the new array
// from the function.
const presidents = [
...
/*let newList = presidents.forEach(newArray);


let newArray = presidents.forEach(president => {
    let newList = president[1].push() + president[2].push();
    console.log(newList);
});

presidents.forEach(newArray);*/

let fn = () => {
    let copy = [];//locally scoped array
